...
NBSS.plot <- function(biovolume_nbss,ESD=TRUE){
  ...
  sample_name <- biovolume_nbss[,unique(sample_id)] #I THINK THIS SEPARATES BY SAMPLE
  data <- NBSS(biovolume_nbss,N=TRUE) #CALLING PREVIOUS FUNCTION
  
  #convert back to ESD :
  
  data[,Spectra:=2*10**3*(Spectra*3/(4*pi))**(1/3)] #I don't follow what's happening here, why would we convert ESD just to convert it back?
  
  min_mesh <- unique(biovolume_nbss$acq_min_mesh) #QUESTION: WE HAVE THIS IN THE ORIGINAL DATA SET AND I CAN BRING IT INTO THE NEW ONE, BUT SINEC WE ARE DOING SEPARATE NBSS FOR 200 AND 700 COULD THIS BE SUBSTITUTED FOR MIN AND MAX ESD?
  max_mesh <- unique(biovolume_nbss$acq_max_mesh)
