...
for (yy in sort(unique(monthly_nbss$Year))) {
  ...
  for (mm in sort(unique(yearly_data$Month))) {
    ...
    monthly_straight <- subset(data3, Spectra > 700 & Spectra < 3000)
    ...
    p <- ggplot(monthly_straight, aes(x = Spectra, y = NBSS)) +
      geom_point(data = data3, aes(Spectra, NBSS), color = "black") +
      geom_point(color = "red") +
      scale_x_log10(limits = c(min_mesh3, max_mesh3)) +
      labs(
        x = NULL, #"Equivalent Spherical Diameter [µm]",
        y = NULL, #"NBSS log[mL/m³/mm³]",
        title = paste("NBSS", yy, "Month", mm)
      )

    # store plot instead of printing
...
combined_plot
ggsave("combined_plot.png", plot = combined_plot, dpi = 300, width = 14, height = 12)
browseURL("combined_plot.png")
...
```{r, NBSS slopes by month}
slopes_data <- meso_slopes_table %>%
  mutate(std = sd(slope, filter %in% month))
meso_slopes <- ggplot(meso_slopes_table, aes(x = month, y = slope)) +
      #geom_line() +
      geom_point() +
      labs(
        x = "Month", 
        y = "NBSS Slope" #"log(mL/m³/mm³)/[µm]",
      ) +
  theme_minimal() 
print(meso_slopes)
```
