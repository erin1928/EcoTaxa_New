...
data1 <- NBSS(meso_nbss,N=TRUE) #setting normalized NBSS function as dataset "data" 
  ...
  min_mesh1 <- 4/3*pi*(min(data1$Spectra)/2)^3 
  max_mesh1 <- 4/3*pi*(max(data1$Spectra)/2)^3
  
  meso_straight <- data1
 #meso_straight <- meso_straight[meso_straight$Spectra>700,] #selecting only the straight line
  #meso_straight <- meso_straight[meso_straight$Spectra<3000,]
  meso_straight$Spectra <-  4/3*pi*(meso_straight$Spectra/2)^3 #mm3
  meso_straight <- meso_straight[meso_straight$Spectra>10^-3,] #selecting only the straight line
  meso_straight <- meso_straight[meso_straight$Spectra<10^3,]
  meso_plot <- ggplot(meso_straight, aes(x = Spectra, y = NBSS))+ 
...
model1 <- lm(NBSS ~ log(Spectra), data = meso_straight)
summary(model1)


meso_plot + geom_abline(intercept = model1$coefficients[1], slope = model1$coefficients[2], color = "red")
