...
biovolume_nbss <- Biovolume_df %>%
  filter(
    ...
    Station %in% c("ram", "ram_island", "nan")
  ) %>%
  group_by(Date, Fraction, Volume, Tow, Station) %>%
  mutate(
    biovol_conc = if_else(
      Fraction == 1,
      Spherical_volume / (Volume / 1e6),          # cm³/m³
      (Spherical_volume * (Fraction / 2)) / (Volume / 1e6)  # apply splitting correction
    )
  ) %>%
  # Exclude specific dates from the pipeline due to only doing 1 scan
  mutate(
    biovol_conc = if_else(
      Date %in% as.Date(c("2024-08-06", "2025-10-27")),  # use correct column name
      biovol_conc * 2,
      biovol_conc
    )
  ) %>%
...
library(writexl)
# Get unique dates
unique_dates <- unique(biovolume_nbss$Date)
print(unique_dates)
rm(unique_dates)
...
data1 <- NBSS(meso_nbss,N=TRUE) #setting normalized NBSS function as dataset "data" 
 ...
 meso_straight_esd <- meso_straight_esd[meso_straight_esd$Spectra>1e-01,] #selecting only the straight line
  ...
  #model1 <- lm(NBSS ~ log10(Spectra), data = meso_straight_esd, subset = -outliers)
model1 <- lm(NBSS ~ log10(Spectra), data = meso_straight_esd)
summary(model1)
plot(model1)

pred_df <- (meso_straight_esd) %>%
  arrange(Spectra) %>%  # ensure the line is plotted correctly
  mutate(predicted_NBSS = predict(model1, newdata = .))
  meso_plot_esd <- ggplot(meso_straight_esd, aes(x = Spectra, y = NBSS))+ 
  geom_point(data = data1, aes(x = Spectra, y = NBSS), color = "black")+
    geom_point(color = "red") +
  geom_line(data = pred_df, aes(x = Spectra, y = predicted_NBSS), color = "red", size = 0.75) +
...
ggsave("NBSS_meso.png", plot = meso_plot_esd, dpi = 300)
...
meso_straight_esd[outliers, ]

missing_nbss <- biovolume_nbss %>% filter(Tow == "200", Date == "2024-06-13")
datax <- NBSS(missing_nbss,N=TRUE) 
min_meshx <- min(datax$Spectra)
  max_meshx <- max(datax$Spectra)
meso_plot_esdx <- ggplot(datax, aes(x = Spectra, y = NBSS))+ 
  geom_point(data = datax, aes(x = Spectra, y = NBSS), color = "black")+
    #geom_point(color = "red") +
  #geom_line(data = pred_df, aes(x = Spectra, y = predicted_NBSS), color = "red", size = 0.75) +
  scale_x_log10(limits = c(min_meshx, max_meshx)) +
  labs(
    x = "Volume (mm³)",
   # x = "Equivalent Spherical Diameter [µm]",
    y = "NBSS log[mL/m³/mm³]",
...
data2 <- NBSS(macro_nbss,N=TRUE) #setting normalized NBSS function as dataset "data" 
 # data2[,Spectra:=2*10**3*(Spectra*3/(4*pi))**(1/3)] #ESD
  ...
  macro_straight_esd <- macro_straight_esd[macro_straight_esd$Spectra>0.65,] #selecting only the straight line
  macro_straight_esd <- macro_straight_esd[macro_straight_esd$Spectra<1e+03,]
