...
data2 <- NBSS(macro_nbss,N=TRUE) #setting normalized NBSS function as dataset "data" 
  ...
  labs(
    x = "Equivalent Spherical Diameter [µm]",
    y = "NBSS log[mL/m³/mm³]", #"NBSS [mm³/mm³/m³]" original but i think that is wrong
    title = "NBSS across samples"
  )
...
model2 <- lm(NBSS ~ Spectra, data = data2)
summary(model2)
# Extract coefficients
...
monthly_nbss <- biovolume_nbss %>% 
  mutate(
    Month = as.month(Date),
    filter(Tow == "200"))
data3 <- NBSS(monthly_nbss,N=TRUE) #setting normalized NBSS function as dataset "data" 
  data3[,Spectra:=2*10**3*(Spectra*3/(4*pi))**(1/3)]

  min_mesh2 <- min(350) #data2$Spectra)*1.5 - not working as intended, multiplied 700*1.5 and added that as minimum
  # min(large_nbss$object_esd) #micro m -- JP not really of mesh but of particles

  max_mesh3 <- max(data3$Spectra)
  monthly_nbss <- ggplot(data3, aes(x = Spectra, y = NBSS))+ 
  geom_point() +
  scale_x_log10(limits = c(min_mesh3, max_mesh3)) +
  labs(
    x = "Equivalent Spherical Diameter [µm]",
    y = "NBSS log[mL/m³/mm³]", #"NBSS [mm³/mm³/m³]" original but i think that is wrong
    title = "NBSS For Each Month"
  )
