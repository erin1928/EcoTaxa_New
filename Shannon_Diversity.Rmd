...
sd_df <- object_counts_per_m3_by_date %>% #all detritus has been removed and count values are normalized for the volume of water moving through
 mutate(
...
#Shannon by Chl A
# Plot Shannon diversity by month
sd_box_plot_chl <- ggplot(
  shannon_with_season_gam,
  aes(
    x = avg_chl_a,
    y = shannon_index
  )
...
print(sd_box_plot_chl)

sd_chl_line <- ggplot(
  data = shannon_with_season_gam,
  aes(x = log(avg_chl_a), y = shannon_index, color = year(shannon_with_season_gam$object_date))
) +
...
sd_temp_line <- ggplot(
  data = shannon_with_season_gam,
  aes(x = Water_Temp_Avg, y = shannon_index)
) +
  #geom_line(color = "black", linewidth = 0.7) +
