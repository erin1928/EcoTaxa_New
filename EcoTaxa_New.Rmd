...
object_counts_per_m3_by_date <- object_counts_per_m3_by_date %>%
  ...
  summarise(
    n_cor_ind_m3 = sum(n_cor_ind_m3, na.rm = TRUE),
    .groups = "drop"
  )
...
# Get unique dates
unique_station <- unique(combined_df$sample_stationid)
print(unique_station)
rm(unique_station)

# Get unique dates
unique_mesh <- unique(combined_df$sample_net_mesh)
print(unique_mesh)
rm(unique_mesh)
#library(writexl)
...
library(scales)
# Plot for Tow = 200
biovolume_plot_200 <- ggplot(data = object_counts_per_m3_by_date #%>% filter(Tow %in% as.numeric(200), Station %in% c("ram", "ram_island", "nan", "NaN") )
