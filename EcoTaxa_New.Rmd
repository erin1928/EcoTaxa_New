...
# Give this extraction a unique subfolder
extracted_dir <- "EcoTaxa_Test/Aug_Sept"
...
#combined_df_2 <- combined_df_2[!duplicated(combined_df_2), ] #gets rid of exact duplicates
...
count_unique_2 <- length(unique(combined_df_unique$object_id))
print(count_unique_2)
count_date <- length(unique(combined_df_unique$object_date))
print(count_date)
```
...
Spherical_Volume <- ((4/3)*pi*(((combined_df_unique$object_esd/2)*combined_df_unique$process_particle_pixel_size_mm)^3))/1000 #ESD in pixels to cm^3 divide by 1000 to convert to mL (vol 1 individual)
#Divided by 2 to get radius
Biovolume <- ((Spherical_Volume*(combined_df_unique$acq_sub_part)) / (combined_df_unique$sample_tot_vol/(100^3)))
```
...
Biovolume_df <- data.frame(
  Biovolume = Biovolume,  # Replace this with your Biovolume vector
  Date = combined_df_unique$object_date,  # Date from object_date column
  Fraction = combined_df_unique$acq_sub_part, # Include acq_sub_part here
  Volume = combined_df_unique$sample_tot_vol,# Include sample_net_mesh here
  Spherical_volume = Spherical_Volume,
  Tow = combined_df_unique$sample_net_mesh, # Include sample_net_mesh here
  Station = combined_df_unique$sample_stationid, #Allow for selection of specific sampling sites
  object_id = combined_df_unique$object_id
 )
...
object_counts_og <- combined_df_unique %>%
  filter(
...
object_counts_og_with_dates <- combined_df_unique %>%
  filter(
    sample_net_mesh %in% c("200"),
    sample_stationid %in% c("ram", "ram_island", "na", "NaN")
  ) %>%
...
```{r getting count data for time series and keeping things in their date, include=FALSE}
# Final summarization with object_date included
object_counts_og_with_dates_no_detritus <- combined_df_unique %>%
  filter(
    sample_net_mesh %in% c("200"),
...
object_counts_detritus <- combined_df_unique %>%
  filter(
    sample_net_mesh %in% c("200"),
    sample_stationid %in% c("ram", "ram_island", "na", "NaN")) %>%
  group_by(object_date, object_annotation_category, sample_net_mesh, sample_stationid) %>%
