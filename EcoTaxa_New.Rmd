...
object_counts_per_m3_by_date <- combined_df %>%
  ...
  summarise(
    n = n(),
    acq_sub_part = first(acq_sub_part), #This will multiple the n count value for each date with the correct splitting fraction
    n_cor = n * acq_sub_part,
    sample_tot_vol = first(sample_tot_vol),
    n_cor_ind_m3 = (n_cor/(sample_tot_vol/1e6)), #normalizes individuals to individuals per m^3 (sample_tot_vol is in cm^3)
    .groups = "drop"
  ) %>%
