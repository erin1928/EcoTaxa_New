...
combined_df$object_date[combined_df$object_date == as.Date("2024-06-13") & combined_df$sample_net_mesh == 200 & combined_df$acq_sub_part == 32 & combined_df$sample_tot_vol == 68382.99] <- as.Date("2024-06-27")
...
combined_df$object_date[combined_df$object_date == as.Date("2024-08-07")] <- as.Date("2024-08-06")
...
combined_df$acq_sub_part[combined_df$object_date == as.Date("2024-06-13") & as.double(combined_df$acq_sub_part) == 32 & combined_df$sample_tot_vol == 	106860.337] <- 64
...
print(Biovolume_df)
unique_dates <- unique(Biovolume_df$Date)
print(unique_dates)
...
object_counts_per_m3_by_date <- combined_df %>%
  filter(
    ...
    !object_annotation_category %in% c("bubble"),
    !object_annotation_category %in% c("seaweed"), 
    !object_annotation_category %in% c("Balanus<Balaninae"),
    !object_annotation_category %in% c("nauplii-molt"),
    !object_annotation_category %in% c("Insecta"),
    !object_annotation_category %in% c("ctenophore-freagments"),
    !object_annotation_category %in% c("medusae"), 
    !object_annotation_category %in% c("molt"), 
    !object_annotation_category %in% c("egg-cluster"),
...
mutate(
  ...
  mutate(
    object_annotation_category = case_when(
      object_annotation_category %in% c("Acartia", "Acartia (Acartiura) hudsonica", "Acartia (Acanthacartia) tonsa") ~ "Acartia spp.",
  ...
  mutate(
    object_annotation_category = case_when(
      object_annotation_category %in% c("Pseudocalanus", "Pseudocalanus", "Pseudocalanus elongatus", "pseudocalanus-sp-with-eggs") ~ "Psuedocalanus spp.",
      TRUE ~ object_annotation_category
    )
  ) %>%
  ...
  mutate(
    object_annotation_category = case_when(
      object_annotation_category %in% c("Paracalanus", "Paracalanus parvus", "	paracalanus+sp+eggs") ~ "Paracalanus spp.",
      TRUE ~ object_annotation_category
    )
  ) %>%
  ...
  mutate(
   ...
   mutate(
    object_annotation_category = case_when(
      object_annotation_category %in% c("Oithona") ~ "Oithona spp.",
      TRUE ~ object_annotation_category
    )
  ) 
...
object_counts_per_m3_by_date <- object_counts_per_m3_by_date %>%
 # group_by(object_date) %>%
  #summarise(
   # n_cor_ind_m3 = sum(n_cor_ind_m3) *
     # if_else(object_date <= as.Date("2025-08-06"), 1.007, 1),
    #.groups = "drop"
  #)
  ...
  summarise(
   n_cor_ind_m3 = sum(n_cor_ind_m3),
   if_else(object_date <= as.Date("2025-08-06"), 1.007, 1),#add it all up
    .groups = "drop"
  )
...
biovolume_plot_200 <- ggplot(
  ...
  aes(x = object_date, y = log(n_cor_ind_m3))
) +
