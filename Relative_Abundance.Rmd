...
object_counts_og_with_dates_no_detritus <- combined_df %>%
  ...
  n_cor = if_else(
    ...
    n_cor*2,      # keep existing value
    n_cor),
  n_cor_ind_m3 = (n_cor / (sample_tot_vol / 1e6))*1.007
    ) %>%
  ...
  mutate(
    object_annotation_category = case_when(
     ...
     # object_annotation_category %in% c("Pseudocalanus sp.", "Pseudocalanus elongatus", "pseudocalanus-sp-with-eggs") ~ "Psuedocalanus spp.",
     ...
     # object_annotation_category %in% c("Evadne", "evadne-nordmanni-multiple") ~ "Evadne spp.",
      ...
      object_annotation_category %in% c("Copepoda<Maxillopoda", "copepod shape", "badfocus<Copepoda", "multiple<Copepoda", "Microcalanus", "Microcalanus sp.", "Parvocalanus", "Temora longicornis", "Tortanus sp.", "Calanoida", "Calanus sp.", "copepod sp.", "Paracalanus sp.", "Paracalanus parvus", "paracalanus+sp+eggs", "Centropages", "Centropages hamatus", "Centropages typicus", "Acartia", "Acartia hudsonica", "Acartia tonsa", "Eurytemora affinis", "eurytemora affinis-male", "eurytemora affinis-female", "eurytemora-with-eggs", "Pseudocalanus sp.", "Pseudocalanus elongatus", "pseudocalanus-sp-with-eggs", "Oithona sp.", "Metridia", "Labidocera") ~ "Copepoda spp.",
      object_annotation_category %in% c("Aurelia sp.", "balanus-larvae", "Beroe cucumis", "Bivalvia larvae", "Chaetognatha", "Ctenophora sp.", "Decapoda", "Carcinus maenas", "Paguridae", "zoe<Decopoda", "Euphausiacea larvae", "Hydrozoa", "Obelia sp.", "Rathkea octopunctata", "Sarsia tubulosa", "Polychaeta", "polychaete larva", "egg<other", "fish egg", "larval-fishes", "multiple<other", "multiple+copepoda+detritus", "detritus-copepodo", "Leuckartiara", "Caligus", "Upogebia", "Crangon", "Anapagurus", "Harpacticoida", "Euphausiidae", "Gammarus", "Podocoryna") ~ "Other zooplankton",
      TRUE ~ object_annotation_category
    )
  ) %>%
  # mutate(
...
calculate_top10_relative_abundance_no_detritus <- function(df, exclude_categories = NULL) {
  ...
  if (is.null(exclude_categories)) {
    exclude_categories <- c(
      ...
      "detritus-flat", "nauplii-molt", "Balanus<Cirripedia", "Insecta", "egg-cluster", "scale"
    )
  }
