...
object_counts_og_with_dates_no_detritus <- combined_df %>%
  ...
  mutate(
    object_annotation_category = case_when(
      ...
      object_annotation_category %in% c("Evadne", "evadne-nordmanni-multiple") ~ "Evadne spp.",
      object_annotation_category %in% c("Copepoda<Maxillopoda", "copepod shape", "badfocus<Copepoda", "multiple<Copepoda", "Microcalanus", "Microcalanus sp.", "Parvocalanus", "Temora   longicornis", "Tortanus sp.", "Calanoida", "Calanus sp.", "copepod sp.") ~ "Copepoda spp.",
      object_annotation_category %in% c("Aurelia sp.", "balanus-larvae", "Beroe cucumis", "Bivalvia larvae", "Chaetognatha", "Ctenophora sp.", "Decapoda", "Carcinus maenas", "Paguridae", "zoe<Decopoda", "Euphausiacea larvae", "Hydrozoa", "Obelia sp.", "Rathkea octopunctata", "Sarsia tubulosa", "Polychaeta", "polychaete larva", "egg<other", "fish egg", "larval-fishes", "medusae", "multiple<other", "multiple+copepoda+detritus", "detritus-copepodo") ~ "Other zooplankton",
      TRUE ~ object_annotation_category
    )
  ) %>%
   mutate(
...
calculate_top10_relative_abundance_no_detritus <- function(df, exclude_categories = NULL) {
  ...
  if (is.null(exclude_categories)) {
    exclude_categories <- c(
      ...
      "detritus-flat", "nauplii-molt", "Balanus<Cirripedia", "Insecta", "egg-cluster"
    )
  }
...
seasonal_abundance_plot <- ggplot(top10_relative_abundance_season_df_no_detritus,
                                 aes(x = factor(season, levels = c("Winter", "Spring", "Summer", "Fall")), y = relative_abundance, fill = object_annotation_category)) +
  facet_wrap(~ year) +
  geom_bar(stat = "identity") +
  labs(
    x = "Season",
