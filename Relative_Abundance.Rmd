...
object_counts_og <- combined_df %>%
  filter(
    ...
    sample_stationid %in% c("ram", "ram_island", "nan", "NaN"),
...
object_counts_og_with_dates_no_detritus <- combined_df %>%
  filter(
    sample_net_mesh %in% c("200"),
    sample_stationid %in% c("ram", "nan", "NaN", "ram_island"),
    !object_annotation_category %in% c("badfocus<other"), 
    !object_annotation_category %in% c("scales"),
    !object_annotation_category %in% c("scale"),
  ...
  mutate(
    object_annotation_category = case_when(
      ...
      #object_annotation_category %in% c("Podonidae") ~ "Podon spp.",
      #object_annotation_category %in% c("Centropages", "Centropages hamatus", "Centropages typicus") ~ "Centropages spp.",
      object_annotation_category %in% c("unidentified-nauplii", "nauplii<Copepoda") ~ "Copepod nauplii",
     # object_annotation_category %in% c("Pseudocalanus", "Pseudocalanus elongatus", "pseudocalanus-sp-with-eggs") ~ "Psuedocalanus spp.",
     # object_annotation_category %in% c("Paracalanus", "Paracalanus parvus", "paracalanus+sp+eggs") ~ "Paracalanus spp.",
     # object_annotation_category %in% c("Eurytemora affinis", "eurytemora affinis-male", "eurytemora affinis-female", "eurytemora-with-eggs") ~ "Eurytemora spp.",
     # object_annotation_category %in% c("Evadne", "evadne-nordmanni-multiple") ~ "Evadne spp.",
      object_annotation_category %in% c("Evadne", "evadne-nordmanni-multiple", "Podonidae") ~ "Cladocera spp.",
      object_annotation_category %in% c("Copepoda<Multicrustacea", "copepod shape", "badfocus<Copepoda", "multiple<Copepoda", "Microcalanus", "Microcalanus sp.", "Parvocalanus", "Temora longicornis", "Tortanus sp.", "Calanoida", "Calanus", "copepod sp.", "Paracalanus", "Paracalanus parvus", "paracalanus+sp+eggs", "Centropages", "Centropages hamatus", "Centropages typicus", "Acartia", "Acartia (Acartiura) hudsonica", "Acartia (Acanthacartia) tonsa", "Eurytemora affinis", "eurytemora affinis-male", "eurytemora affinis-female", "eurytemora-with-eggs", "Pseudocalanus", "Pseudocalanus elongatus", "pseudocalanus-sp-with-eggs", "Oithona", "Metridia", "Labidocera", "Paracalanus sp.", "Pseudocalanus sp.", "Tortanus", "Calanus", "Oithona sp.") ~ "Copepoda spp.",
      object_annotation_category %in% c("Aurelia", "balanus-larvae", "Beroe cucumis", "Bivalvia<Mollusca", "Chaetognatha", "Ctenophora<Animalia", "Decapoda", "Carcinus maenas", "Paguridae", "zoe<Decopoda", "Euphausiacea larvae", "Hydrozoa", "Obelia", "Rathkea octopunctata", "Sarsia tubulosa", "Polychaeta", "polychaete larva", "egg<other", "fish egg", "larval-fishes", "multiple<other", "multiple+copepoda+detritus", "detritus-copepodo", "Leuckartiara", "Caligus", "Upogebia", "Crangon", "Anapagurus", "Harpacticoida", "Euphausiidae", "Gammarus", "Podocoryna", "Ctenophora sp.", "Obelia sp.","Euphausiacea", "Aurelia sp.", "Bivalvia larvae") ~ "Other zooplankton",
      object_annotation_category %in% c("Gastropoda larvae", "Gastropoda<Mollusca")~ "Gastropoda",
      TRUE ~ object_annotation_category
    )
  ) %>%
  # mutate(
    #object_annotation_category = case_when(object_annotation_category %in% c("Oithona sp.") ~ "Oithona spp.",
     # TRUE ~ object_annotation_category
   # )
 # ) %>%
  #Group by object_date too
...
calculate_top10_relative_abundance <- function(df, exclude_categories = NULL) {
  df %>%
    ...
    summarise(total_n_cor_ind_m3 = sum(n_cor_ind_m3, na.rm = TRUE), .groups = "drop") %>%
    
    # Calculate total n_cor per month for relative abundance
    group_by(month) %>%
    mutate(
      relative_abundance_ind_m3 = total_n_cor_ind_m3 / sum(total_n_cor_ind_m3, na.rm = TRUE)
    ) %>%
    
    # For each month, keep only top 10 species by relative abundance
    arrange(month, desc(relative_abundance_ind_m3)) %>%
    group_by(month) %>%
    slice_head(n = 10) %>%
    ungroup()
}
...
monthly_abundance_plots <- ggplot(top10_relative_abundance_df, aes(x = format(month, "%Y-%m"), y = relative_abundance_ind_m3, fill = object_annotation_category)) +
  geom_bar(stat = "identity") +
  labs(
    x = "Month",
    y = "Relative Abundance (%)",
...
calculate_top10_relative_abundance_no_detritus <- function(df, exclude_categories = NULL) {
  ...
  if (is.null(exclude_categories)) {
    exclude_categories <- c(
      "badfocus<other", "Macroalgae", "scales", "detritus", "bubble", 
      "ctenophore-freagments", "molt", "medusae", "fiber<detritus", 
      "copepod carapace", "detritus-green", "detritus-organic", "shadow", 
      "detritus-flat", "nauplii-molt", "Balanus<Cirripedia", "Insecta", "egg-cluster", "scale", "seaweed"
    )
  }
  
  df %>%
    # Remove unwanted categories
    filter(!object_annotation_category %in% exclude_categories) %>%
    
    # Extract month
    mutate(month = floor_date(as.Date(object_date), "month")) %>%
    
    # Sum counts by month and species using normalized count data
    group_by(month, object_annotation_category) %>%
    summarise(total_n_cor_ind_m3 = sum(n_cor_ind_m3), .groups = "drop") %>%
    
    # Calculate relative abundance across all species
    group_by(month) %>%
    mutate(relative_abundance_ind_m3 = total_n_cor_ind_m3 / sum(total_n_cor_ind_m3)) %>%
    
    # Keep only top 10 species per month by relative abundance
    arrange(month, desc(relative_abundance_ind_m3)) #%>%
    #slice_head(n = 10) %>%
    ...
    #object_annotation_category =
      f#orcats::fct_reorder(object_annotation_category, relative_abundance_ind_m3)
  #)
}
...
ggsave("monthly_abundance_plots_no_detritus.png", plot = monthly_abundance_plots_no_detritus, dpi = 300)
browseURL("monthly_abundance_plots_no_detritus.png")


##====CLEANED====

species_colors5 <- c(
  "Cladocera spp."        = "#1A1A1A",  # near-black
  "Copepoda spp."       = "#4D4D4D",  # dark grey
  "Copepod nauplii"         = "#8C8C8C",  # mid grey (replaces white)
  "Other zooplankton"     = "#C7C7C7",  # light grey
  "Gastropoda larvae"     = "white"   # muted blue accent
)
monthly_abundance_plots_no_detritus_2 <-
  ggplot(
    top10_relative_abundance_df_no_detritus,
    aes(
      x = month,
      y = relative_abundance_ind_m3,
      fill = object_annotation_category
    )
  ) +
  geom_col(width = 25.0, color = "black", linewidth = 0.5) +
  ...
  scale_fill_manual(values = species_colors5) +
...
print(monthly_abundance_plots_no_detritus_2)
ggsave("monthly_abundance_plots_no_detritus_2.png", plot = monthly_abundance_plots_no_detritus_2, dpi = 300)
browseURL("monthly_abundance_plots_no_detritus_2.png")
```
...
calculate_top10_relative_abundance_season_no_detritus <- function(df, exclude_categories = NULL) {
  df %>%
    ...
    mutate(
      relative_abundance_m3 = total_n_cor_ind_m3 / sum(total_n_cor_ind_m3, na.rm = TRUE)
    ) %>%
    ...
    !object_annotation_category %in% c("bubble"),
    !object_annotation_category %in% c("Macroalgae"), 
    !object_annotation_category %in% c("Balanus<Cirripedia"),
    !object_annotation_category %in% c("nauplii-molt"),
    !object_annotation_category %in% c("Insecta"),
    !object_annotation_category %in% c("ctenophore-freagments"),
    !object_annotation_category %in% c("medusae"), 
    !object_annotation_category %in% c("molt"), 
    !object_annotation_category %in% c("egg-cluster")) %>%
    arrange(season, desc(relative_abundance_m3)) %>%
    group_by(season, year) #%>%
    #slice_head(n = 10) %>%
    #ungroup()
}
...
top10_relative_abundance_season_df_no_detritus<- 
  top10_relative_abundance_season_df_no_detritus %>%
  mutate(abundance_percentage = 100 * relative_abundance_m3)
...
seasonal_abundance_plot <- ggplot(top10_relative_abundance_season_df_no_detritus,
                                 aes(x = factor(season, levels = c("Winter", "Spring", "Summer", "Fall")), y = relative_abundance_m3, fill = object_annotation_category)) +
  facet_wrap(~ year) +
  geom_bar(stat = "identity") +
  labs(
    x = "Season",
...
seasonal_abundance_plot_2 <-
  ...
  scale_fill_manual(values = species_colors5, drop = FALSE) +
...
print(seasonal_abundance_plot_2)
ggsave("seasonal_abundance_plot_2.png", plot = seasonal_abundance_plot_2, dpi = 300)
browseURL("seasonal_abundance_plot_2.png")
```

#GAMs for aundances
```{r, GAM by month}
library(mgcv)
pca_df_ra <- pca_df %>% mutate(month = floor_date(as.Date(Date), "month"))
# Join with season info (assuming season is the same for each object_date)
ra_gam <- pca_df_ra %>%
  left_join(pca_gam %>% select(month, avg_chl_a, Water_Temp_Avg, Salinity_Avg, Secchi.depth) %>% distinct(), by = "month") %>%
...
ra_gam <- na.omit(ra_gam)
# Fit model
gam_1 <- gam(relative_abundance_m3 ~ s(avg_chl_a) + s(Water_Temp_Avg) + s(Salinity_Avg) +s(Secchi.depth) +s(Month), family = gaussian(), data = shannon_with_season_gam)
...
```{r total number of species and their counts collectively throughout entire time series no detritus}

object_counts_og_with_dates_no_detritus <- object_counts_og_with_dates_no_detritus %>%
  mutate(year = year(object_date))
calculate_relative_abundance_no_detritus <- function(df, exclude_categories = NULL) {
  df %>%
    # Filter out the excluded categories if provided
    { if (!is.null(exclude_categories)) filter(., !object_annotation_category %in% exclude_categories) else . } %>%
...
relative_abundance_df_no_detritus <- calculate_relative_abundance_no_detritus(
  object_counts_og_with_dates_no_detritus,
)
print(relative_abundance_df_no_detritus)
```